...
def test_no_duration_binds_extracted_audio_and_empty_result(monkeypatch, tmp_path):
    video = _video(tmp_path)
...
def test_audio_extraction_failure_writes_failure_evidence(monkeypatch, tmp_path):
    video = _video(tmp_path)
    ...
    monkeypatch.setattr(asr, "run_cmd", fail_with_partial)
...
def test_audio_extraction_exception_cannot_leave_partial_audio(monkeypatch, tmp_path):
    video = _video(tmp_path)
    monkeypatch.setitem(asr.CONFIG, "mimo_asr_api_key", "test-key")

    def raise_with_partial(cmd, **_kwargs):
        Path(cmd[-1]).write_bytes(b"partial")
        raise OSError("ffmpeg launch failed")

    monkeypatch.setattr(asr, "run_cmd", raise_with_partial)
    with pytest.raises(RuntimeError, match="音频提取失败"):
        asr.transcribe_audio(video, tmp_path)
    assert not (tmp_path / "audio.wav").exists()
    assert _read_evidence(tmp_path)["status"] == "FAILED_AUDIO_EXTRACTION"
...
def test_empty_provider_text_is_unknown_not_proven_silence(monkeypatch, tmp_path):
    video = _video(tmp_path)
    monkeypatch.setitem(asr.CONFIG, "mimo_asr_api_key", "test-key")
    monkeypatch.setattr(asr, "run_cmd", _successful_extract())
...
def test_cache_requires_valid_evidence_and_detects_tampering(tmp_path):
    video = _video(tmp_path)
    result_path = tmp_path / "asr_result.json"
    result = [{"start": 0.0, "end": 1.0, "text": "hello"}]
    ...
    write_asr_timing_evidence(
        ...
        video,
        "AVAILABLE_COARSE",
        observed_segments=result,
        final_segments=result,
        audio_path=audio,
    )
    assert _asr_cache_state(result_path, meta, video) == "FRESH"

    evidence = _read_evidence(tmp_path)
    evidence["asr_result_fingerprint"] = "tampered"
    (tmp_path / EVIDENCE_FILENAME).write_text(json.dumps(evidence), encoding="utf-8")
...
def test_sidecar_rejects_changed_bound_artifact(tmp_path, changed):
    ...
    result_path.write_text(json.dumps(segments), encoding="utf-8")
    ...
    {"source": video, "audio": audio, "result": result_path}[changed].write_bytes(
        b"changed-content-with-a-different-size"
    )
...
def test_glossary_change_invalidates_fresh_corrected_cache(tmp_path):
    ...
    corrected = [{"start": 0.0, "end": 1.0, "text": "叶轻眉"}]
    research = tmp_path / "background_research.json"
    research.write_text(
        json.dumps({"characters": {"叶轻眉": "角色"}}, ensure_ascii=False),
        encoding="utf-8",
    )
    result_path.write_text(json.dumps(corrected, ensure_ascii=False), encoding="utf-8")
    meta = _asr_cache_payload(video)
    _write_stage_meta(result_path, meta)
    # AVAILABLE_COARSE requires a bound audio artifact.
    audio = tmp_path / "audio.wav"
    audio.write_bytes(b"RIFF-audio")
    (tmp_path / "audio.wav.meta.json").write_text(
        json.dumps({"source_video_fingerprint": asr.file_fingerprint(video)}),
...
def test_no_duration_result_is_not_reusable_cache(monkeypatch, tmp_path):
    video = _video(tmp_path)
    monkeypatch.setitem(asr.CONFIG, "mimo_asr_api_key", "test-key")
    monkeypatch.setattr(asr, "run_cmd", _successful_extract())
    monkeypatch.setattr(asr, "get_video_duration", lambda _path: 0.0)
...
def test_empty_transcription_is_not_a_permanent_cache_hit(monkeypatch, tmp_path):
    video = _video(tmp_path)
    monkeypatch.setitem(asr.CONFIG, "mimo_asr_api_key", "test-key")
    monkeypatch.setattr(asr, "run_cmd", _successful_extract())
    monkeypatch.setattr(asr, "get_video_duration", lambda _path: 1.0)
...
def _valid_available_evidence(tmp_path):
    video = _video(tmp_path)
    audio = tmp_path / "audio.wav"
    audio.write_bytes(b"RIFF-audio")
    (tmp_path / "audio.wav.meta.json").write_text(

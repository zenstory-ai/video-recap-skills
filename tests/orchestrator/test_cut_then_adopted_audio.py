...
def test_reordered_multi_cut_reuses_wav_through_existing_full_adoption(tmp_path):
    ...
    first_manifest = json.loads(
        (tmp_path / "adopt-current/assembly_manifest.json").read_text()
    )
    ...
    current_bed = json.loads(current_receipt.read_text())
    reordered_bed = json.loads(reordered_receipt.read_text())
    assert (
        current_bed["outputs"]["prepared_bed.wav"]["pcm_payload_sha256"]
        != (reordered_bed["outputs"]["prepared_bed.wav"]["pcm_payload_sha256"])
    )

    binding = json.loads((final_work / "audio_mix_binding.json").read_text())
    ...
    narration_binding = json.loads(
        (final_work / "narration_input_binding.json").read_text()
    )
    assert narration_binding["segments"][0]["original"]["sha256"] == wav_hash
    manifest = json.loads((final_work / "assembly_manifest.json").read_text())
    assert json.loads(meta.read_text())["segments"][0]["start"] == pytest.approx(0.25)
    ...
    timeline = json.loads((final_work / "timeline.json").read_text())
    ...
    srt = (final_work / "subtitles.srt").read_text()

...
已有采用的精确剪点、只回原片取宽或改构图时，改用显式 `--picture-plan` 路径，详见 [锁帧画面重建](references/locked-picture.md)。它不运行默认吸附、不生成声音，要求新候选目录；已有快慢放需显式逐输出帧的源帧表，不能根据时长差猜速度。`--normalize-only` 仅预检改轨。

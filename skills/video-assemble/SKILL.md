...
若完整 prepared bed 明确不需要旁白，使用 `references/prepared-audio.md` 的
`render_prepared_audio.py` 直接生成新 AAC 并配回锁定画面。该路径要求显式
master gain、完整 48 kHz 样本、严格 AAC priming/末端和隐藏 pair staging；
它不进入 TTS、环境 BGM、duck、loudnorm 或 limiter 路径，也不声称 AAC 有损
解码后的 PCM 与 master 恒等。
...
对白只保留一个渲染来源：母版已烧录则前景不再画对白；需要模板字体时，改用无烧字母版，
先按 `references/subtitle-track.md` 验证并投影当前字幕轨，再把精确文字和帧边界交给前景生产者。
不要用另一份手填字幕或按字数重新分配已校准 cue。生产者回执只是来源声明；
仍须检查实际出字/消字、字体、标题/花字遮挡及片名卡首尾，不能靠回执认定没有重复字。

包装价值来自稳定、可读、与内容一致的排版，不来自效果数量。先建立统一字体、颜色、描边/阴影和轻量动效；底板、边框、花字与音效只有解决具体可读性或叙事任务时才加入。一个样帧好看不代表全片成立。

### 有限语义主题前景

调用方已有锁定母版、经当前画面/音频/帧时钟验证的字幕轨、哈希固定的本地字体与 RGBA
底板时，可运行 `scripts/render_theme_foreground.py PLAN --output-dir NEW_DIR` 生成标题对、精确
字幕、定时注释和可选标记的透明 PNG 序列，再交给 `compose_foreground.py`。内容计划不接受重复
字幕字段或任意 CSS/HTML/JS；隐藏标记仍保留在状态调度中。完整 v1 schema、字体/透明窗门禁、
浏览器隔离与非审批边界见 `references/theme-foreground.md`。动态尾卡仍由调用方以完整序列提供，
本操作不生产或冻结尾卡。

同一命令加 `--compose` 可直接接现有合成器，输出新目录下的
`composed/foreground.mp4`，不重剪、不重配音、不增加第二份字幕。
全片前景无需片尾卡；若计划预留了片尾，须用 `--endcard` 提供原有合成器格式的
明确素材。

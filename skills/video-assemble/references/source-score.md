...
- `prepared_bed_receipt.json` — hashes, clocks, sample ranges, PCM facts and QC facts.
...
```json
{"kind":"frozen","path":"/local/frozen.wav","sha256":"...","audio_stream":0}
```
...
The receipt preserves the original frozen file hash and the canonical output hash.

## Receipt and failure boundary

`prepared_bed_receipt` schema version 1 records the plan hash, each input path/hash,
selected stream, actual CFR/audio clocks, canonical decoded PCM payload hash,
resolved input/output sample ranges, gains, fades, roles, and each output's file hash,
canonical PCM payload hash, format, sample count, finite status and peak.

Plan/source/score hashes are checked before work, before final composition, and after
composition. Canonical decodes and the source/score beds are identity-sealed before
their consumers run, then rechecked after consumption and immediately before
publication. A selected source range must fit the actual canonical decoded samples;
silence padding can never conceal a short source. WAVs remain hidden staging artifacts
until every output validates and all inputs are still current. FFmpeg failure,
mutation, invalid ranges, non-finite PCM, or an existing target directory cannot
publish final-named beds or a receipt.
Diagnostic command/log/intermediate files may remain in the unique failed directory.

`direct_listening` remains `NOT_CHECKED` and `release_approved` remains false.
Sample identity, peaks and tone tests do not prove dialogue choice, musical taste,
subjective balance, acoustic voice identity, or commercial release readiness.
...
prepared bed without narration, use `render_prepared_audio.py` as documented in
`prepared-audio.md`. Do not feed `prepared_bed.wav` through legacy source ducking or
